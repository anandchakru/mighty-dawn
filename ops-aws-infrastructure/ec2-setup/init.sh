...
if [ -z "$JAR_NAME" ]; then
    ...
    echo "Example: $0 mighty-dawn-api-1.0.123.1-SNAPSHOT.jar"
    exit 1
fi
...
sudo mkdir -p /opt/mighty-dawn/ec2-setup
sudo mkdir -p /opt/mighty-dawn/lib
sudo mkdir -p /opt/mighty-dawn/env
sudo mkdir -p /opt/mighty-dawn/logs
sudo mkdir -p /opt/mighty-dawn/backups
...
# 4. Create mighty-dawn user if not exists
echo "=== Creating mighty-dawn user ==="
if ! id "mighty-dawn" &>/dev/null; then
    sudo useradd -r -s /bin/bash -d /opt/mighty-dawn -c "mighty-dawn API Service" mighty-dawn
    echo "User 'mighty-dawn' created successfully"
else
    echo "✅ User 'mighty-dawn' already exists"
fi
...
echo "Source: s3://mighty-dawn-${ENV_FOR_S3}-ops/ec2-setup/"
echo "Target: /opt/mighty-dawn/ec2-setup/"

# Create target directory if it doesn't exist
sudo mkdir -p /opt/mighty-dawn/ec2-setup

# Sync all files from S3 ec2-setup directory
if sudo aws s3 sync s3://mighty-dawn-${ENV_FOR_S3}-ops/ec2-setup/ /opt/mighty-dawn/ec2-setup/; then
    echo "✅ Successfully synced EC2 setup scripts from S3"
    
    # Make all shell scripts executable
    sudo chmod +x /opt/mighty-dawn/ec2-setup/*.sh
    
    # Set ownership for all files
    sudo chown -R mighty-dawn:mighty-dawn /opt/mighty-dawn/ec2-setup/
    
    echo "📋 Downloaded EC2 setup files:"
    ls -la /opt/mighty-dawn/ec2-setup/
else
...
if [ -f "/opt/mighty-dawn/ec2-setup/mighty-dawn-api.service" ]; then
    sudo cp /opt/mighty-dawn/ec2-setup/mighty-dawn-api.service /etc/systemd/system/
    sudo systemctl daemon-reload
    sudo systemctl enable mighty-dawn-api.service
    echo "✅ Systemd service configured successfully"
else
    echo "❌ Service file not found: /opt/mighty-dawn/ec2-setup/mighty-dawn-api.service"
    echo "⚠️ Manual service setup may be required"
fi

# 7. Set ownership
echo "=== Setting ownership ==="
sudo chown -R mighty-dawn:mighty-dawn /opt/mighty-dawn
...
APP_SECRET_NAME="mighty-dawn-${ENV_FOR_SECRET}-app-secrets"
...
sudo mv "$TEMP_SPRING_ENV" /opt/mighty-dawn/env/spring.env
sudo chown mighty-dawn:mighty-dawn /opt/mighty-dawn/env/spring.env
sudo chmod 644 /opt/mighty-dawn/env/spring.env

echo "📋 Final Spring environment configuration:"
echo "SPRING_PROFILES_ACTIVE=$(grep SPRING_PROFILES_ACTIVE /opt/mighty-dawn/env/spring.env | cut -d'=' -f2)"
echo "Additional environment variables: $(grep -v SPRING_PROFILES_ACTIVE /opt/mighty-dawn/env/spring.env | wc -l)"
...
if [ -f "/opt/mighty-dawn/lib/app.jar" ]; then
    ...
    sudo cp /opt/mighty-dawn/lib/app.jar /opt/mighty-dawn/lib/$BACKUP_NAME
...
if sudo aws s3 cp "s3://mighty-dawn-${ENV_FOR_S3}-ops/jars/$JAR_NAME" /opt/mighty-dawn/lib/app.jar; then
    echo "✅ Successfully downloaded $JAR_NAME from S3"
    sudo chown mighty-dawn:mighty-dawn /opt/mighty-dawn/lib/app.jar
    sudo chmod 644 /opt/mighty-dawn/lib/app.jar
else
...
# 11. Restart service
echo "=== Restarting mighty-dawn-api service with profile: $SPRING_PROFILE ==="
sudo systemctl restart mighty-dawn-api.service

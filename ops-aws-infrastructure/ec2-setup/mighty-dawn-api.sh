...
# mighty-dawn API Service Startup Script

# Set up logging
LOG_DIR="/opt/mighty-dawn/logs"
LOG_FILE="$LOG_DIR/mighty-dawn-$(date +%Y%m%d).log"
...
log_with_timestamp "Starting mighty-dawn API Service"
...
if [ ! -f "/opt/mighty-dawn/lib/app.jar" ]; then
    log_with_timestamp "ERROR: JAR file not found at /opt/mighty-dawn/lib/app.jar"
    exit 1
fi
...
log_with_timestamp "JAR file: $(ls -la /opt/mighty-dawn/lib/app.jar)"
...
log_with_timestamp "Starting Java application: /opt/mighty-dawn/lib/app.jar $SPRING_ARGS"

# Run the application and pipe all output to our logging function
java -jar /opt/mighty-dawn/lib/app.jar $SPRING_ARGS 2>&1 | while IFS= read -r line; do
